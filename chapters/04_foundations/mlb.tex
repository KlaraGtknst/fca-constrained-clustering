...
Clustering structures data without predefined class labels, which is particularly useful in real-world applications where labeled data is scarce and annotation is costly. 
...
However, pairwise constraints generally apply only at a level of granularity.
...
Formally, both \autoref{def:mlb_implication} and \autoref{def:mlb_counterexample} must hold, expressing that the must-link relation between $d_x$ and $d_y$ is stronger than that between $d_x$ and $d_z$. 
