...
% labelled is BE, labeled is AE

Clustering structures unlabeled data, which is particularly useful in real-world applications where labeled data is scarce and annotation is costly. 
...
An \ac{mlb} constraint $MLB_{x y z} = (d_x, d_y, d_z)$ states that $d_x$ and $d_y$ should be merged before either is merged with $d_z$, where $d_x$ and $d_y$ are symmetric. 
The cluster hierarchy $H$ is defined by $(C, \prec, \text{root})$, where $C$ is a set of clusters, $\prec$ is a strict partial order over $C$ representing the parent-child relation, and $\text{root} \in C$ is the root cluster~\cite{bade_hierarchical_2014}. 
