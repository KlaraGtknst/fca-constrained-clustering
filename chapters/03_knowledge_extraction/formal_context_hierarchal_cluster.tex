\subsection{Formal Context to Hierarchical Clusters}
\label{sec:formal_context_to_hierarchical_cluster}

Given a concept lattice, we can derive a hierarchical cluster structure that is consistent with the implications encoded in the context, by extracting \ac{mlb} constraints from the lattice structure (cf. \autoref{sec:gt_mlb}) and applying them to a hierarchical clustering algorithm, such as \ac{hac} or \ac{ihac}.
\textcolor{red}{TODO}
...
%
% Given a formal context $(G,M,I)$ in \autoref{tab:formal_context} where $G$ denotes the set of data points $\mathcal{D}$ and $M$ denotes the set of clusters $c_i \in \mathcal{C}$, all objects $d \in G$ are leaf nodes of the dendogram in \autoref{fig:dendrogram}.
% Iterative merging of clusters are indicated by agglomerating lines from the bottom to the top in the dendogram.
% \formalcontexttable{}
% \dendogram{}
