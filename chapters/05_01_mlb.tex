...
A collection of subsets $\mathcal{V} \subseteq \mathcal{P}(G)$ is a valid hierarchy regarding $\Sigma$ if: 
\begin{enumerate}
    \item \label{item:valid_hierarchy_root} 
    $G \in \mathcal{V}$ and $\emptyset \in \mathcal{V}$.

    \item \label{item:valid_hierarchy_nested} 
    For any $A, B \in \mathcal{V}$, either $A \cap B = \emptyset$ or $A \subseteq B$ or $B \subseteq A$ (Nested property).
...
Note that Condition~\ref{item:valid_hierarchy_consistency} of \eqref{def:valid_hierarchy} corresponds to \eqref{def:mlb_implication} and Condition~\ref{item:valid_hierarchy_counterexample} of \eqref{def:valid_hierarchy} corresponds to \eqref{def:mlb_counterexample}.
