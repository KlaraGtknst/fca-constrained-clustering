...
In this work, we introduce a method for comparing and integrating knowledge derived from \acs{mlb} constraints with an additional hierarchical knowledge source, such as a topic model. 
...
As our supplementary hierarchical knowledge source, we employ an \acl{lda} topic model, which constructs a document-topic context through statistical inference over the same corpus.

We then analyze the structural differences between the concept lattices guided by the two knowledge sources. %knowledge-base-informed 
